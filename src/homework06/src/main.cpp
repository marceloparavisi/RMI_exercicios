...
//    ROS_INFO("teste: %f", high);
	...
	if (msg->ranges.size() > 0)
	{
		...
		for (int y = 0; y < height; y++)
		{
			...
			//tf::Vector3 target(x, y, 0);
			//float dist = target.distance(tf::Vector3(fromX,fromY,0));
			float angle = acos(v.dot(target)/dist);
...
//			if (dist < 5 )//&& angle > (odom.theta+msg->angle_min) && angle < (odom.theta+msg->angle_max))
			...
			{
				ROS_INFO("%f (%d) angle: %f %f %f",dist, data[y*width+x], (msg->angle_min), angle, (msg->angle_max));
				//if (data[y*width+x] > 0)
				{
